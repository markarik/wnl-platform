...
class InvoicesExport extends Command
{
	...
	public function printLine($invoice, $order, $schema, $valuesOverrides)
	{
		...
		$schema['Imie uczestnika'] = $order->user->first_name;
		$schema['Nazwisko uczestnika'] = $order->user->last_name;
		$schema['Adres uczestnika'] = $order->user->address;
		$schema['Firma?'] = $isCompany ? 'TAK' : 'NIE';
		$schema['Nazwa firmy'] = $isCompany ? $order->user->invoice_name : '';
		$schema['Adres firmy'] = $isCompany ? $order->user->invoice_address : '';
		$schema['NIP firmy'] = $isCompany ? $order->user->invoice_nip : '';

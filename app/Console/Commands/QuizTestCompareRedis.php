...
class QuizTestCompareRedis extends Command
{
    ...
    {
		$byProfile = json_decode(\Storage::get("quiz_test_dump_profile.json"));
		$byUser = collect(json_decode(\Storage::get("quiz_test_dump_user.json")));
		...
		foreach ($byProfile as $record) {
			...
			print "$userId $quizId $score {$dupa[0][2]} ";
			if ($score !== $dupa[0][2]) {
				print 'Whoooooops';
			} else {
				print 'OK';

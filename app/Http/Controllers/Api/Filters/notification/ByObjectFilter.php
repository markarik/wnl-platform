...
abstract class ByObjectFilter extends ApiFilter
{
		...
		return [
			...
			'items' => [
				'by-object' => [
					...
					'message' => 'notification-source',
					'items' => [
						'slides' => $this->getItemsForObjects()
					]
				]
			]
		];
	...
	public function getItemsForObjects() {
		$objects = ['slides', 'qna', 'quiz', 'chat-message'];
		$items = [];
		...
		foreach ($objects as $object) {
			$items->push([
				'name' => $object,
				'value' => '',
				'count' => $this->getCountForObject($object)
			]);
		}
	...
	private function getCountForObject($object) {
		return Notification::search(['data'])
	}

@extends('mail.newsletter.layout')

@section('content')

	<h3>Cześć %recipient_name%!</h3>

	<p><strong>Na wstępie chcielibyśmy jeszcze raz gorąco podziękować Ci za chęć uczestnictwa w warsztatach “Więcej niż LEK”! Mamy nadzieję, że pozwolą Ci nie tylko do jeszcze lepszego przygotowania do egzaminu, ale staną się również źródłem inspiracji do działania w swoim otoczeniu.</strong></p>

	<p>Spotykamy się już w tę sobotę! W związku z tym chcielibyśmy przekazać Ci wszystkie przydatne informacje organizacyjne.</p>

	<h3>Gdzie i kiedy?</h3>
	<p>Warsztaty rozpoczną się w sobotę 4 listopada, <strong>o godzinie 11:00</strong> w <a href="https://goo.gl/maps/f78fwrAzh322">Collegium da Vinci</a>, przy ul. Tadeusza Kutrzeby 20 w Poznaniu. Spotykamy się w sali <strong>N205</strong>, do której będą prowadziły Was od wejścia wskazówki umieszczone na monitorach widocznych w korytarzach Collegium.</p>

	<p>Warsztaty zaplanowane są na 8 godzin, od 11:00 do 19:00, z uwzględnieniem przerw - dwóch 5-minutowych, dwóch 15-minutowych oraz jednej 90-minutowej w porze obiadowej.</p>

	<h3>Jak dojadę?</h3>
	<p>
		<strong>Samochód</strong><br>
		Jeśli dojeżdżasz samochodem, kieruj się na <a href="https://goo.gl/maps/U9MYFYwWZ8R2">bezpłatny parking</a> na tyłach Collegium da Vinci.
	</p>

	<p>
		<strong>Pociąg</strong><br>
		Godzinę rozpoczęcia ustaliliśmy po sprawdzeniu połączeń PKP z głównych kierunków Waszego przyjazdu - Warszawy, Łodzi, Gdańska, Szczecina i Krakowa. Upewniliśmy się, że przy obowiązującym rozkładzie nikt z Was nie będzie miał problemu z dotarciem na czas. :)
	</p>

	<p>
		<strong>Autobus</strong><br>
		Z Dworca PKP, <a href="https://jakdojade.pl/poznan/trasa/z--Dworzec-PKP-Pozna%C5%84-G%C5%82%C3%B3wny--do--Kutrzeby-10?fn=Dworzec%20PKP%20Pozna%C5%84%20G%C5%82%C3%B3wny&tn=Kutrzeby%2010&tc=52.415821:16.931465&fc=52.40221:16.91171&ft=LOCATION_TYPE_POI&tt=LOCATION_TYPE_ADDRESS&d=17.06.17&h=10:45&ia=true&aro=0&t=1&rc=3&ri=2&r=1">co 20 minut kursuje autobus 68</a>, który dowiezie Cię na przystanek “Urząd Marszałkowski” 400m od Collegium da Vinci.
	</p>

	<p>
		<strong>Taxi/Uber</strong><br>
		W Poznaniu możecie bez problemu zamówić Ubera. Jeżeli jednak chcecie wybrać klasyczną taksówkę - polecamy RMI Taxi dostępne pod numerem 1-96-68. Składając zamówienie, poproś o kurs dla kodu <strong>CL87</strong>, aby skorzystać z taryfy rabatowej. :)
	</p>

	<h3>Nie umrę z głodu?</h3>
	<p>Collegium znajduje się 15 minut spacerem od Starego Rynku, w okolicy którego znajdziecie mnóstwo dobrego jedzenia. Będziemy mieli przygotowane dla Was propozycje naszych ulubionych miejsc. :)</p>

	<p>Podczas samych zajęć zadbamy o to, żeby nie zabrakło wody, soków, kawy, herbaty, a także owoców i słodkich przekąsek. ;)</p>

	<h3>Co na warsztatach?</h3>
	<p>Pierwsze spotkanie poświęcimy przede wszystkim efektywnej nauce oraz naszemu przepisowi na LEK. Wierzymy, że pozwoli Ci pełniej wykorzystać cały, 12-tygodniowy kurs. W tygodniu poprzedzającym warsztaty wyślemy jeszcze dokładny plan spotkania, z podziałem na moduły.</p>

<hr style="border: 0; height: 1px; background-image: linear-gradient(to right, rgba(0, 0, 0, 0), rgba(0, 0, 0, 0.75), rgba(0, 0, 0, 0));">

	<p>To już wszystko na dziś! Nie możemy doczekać się naszego spotkania! :)</p>

	<p>Gdybyście mieli jakiekolwiek pytania, zapraszamy do kontaktu pod tym adresem e-mail - <em>warsztaty@wiecejnizlek.pl</em>. Jest on dedykowany dla Was, zapisanych na kurs stacjonarny, i będzie dostępny przez cały czas trwania kursu. :)</p>

	<p>Do zobaczenia!</p>
@endsection

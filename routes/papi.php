...
Route::group(['namespace' => 'Api\PrivateApi', 'middleware' => ['api-auth']], function () {
	...
	Route::group(['middleware' => ['admin']], function () use ($r) {
		...
		Route::put("{$r['products']}/{product}/syncInstalments", 'ProductsApiController@syncInstalments');
	...
	Route::get("{$r['users']}/{id}/{$r['user-profile']}", 'ProfilesApiController@get');
	Route::put("{$r['users']}/{id}/{$r['user-profile']}", 'ProfilesApiController@put');

	Route::get("{$r['profiles']}/.search", "ProfilesApiController@search");
	...
	// Products
	Route::get("{$r['products']}/current/all", 'ProductsApiController@getCurrent');
